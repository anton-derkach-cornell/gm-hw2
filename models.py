...
class MaskedConvnetBlock(nn.Module):
    def __init__(self, filters, *args, **kwargs):
        ...
        self.conv = nn.Sequential(
            MaskedConv2d('B',filters,filters,3,padding=1,bias=False),
...
class VAEEncoder(nn.Module):
    ...
    def forward(self, x, epsilon):
        ...
        for t, flow in enumerate(self.flows):
            ...
            m, s = flow(z, h) 
            z_new = m + s * z 
            log_det_jacobian = torch.log(torch.abs(s)).view(-1, 49).sum(1)  # Compute log |det J|
            logqzx = logqzx + log_det_jacobian  # We add the determinant of the Jacobian because logqzx is the negative log-likelihood probability based on how it was defined initially
            z = z_new
